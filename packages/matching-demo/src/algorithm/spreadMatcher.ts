...
export namespace SpreadMatcher {
    ...
    export interface Result<T extends Entity, P extends Entity> {
        ...
        roundMatches: Match<
            T,
            P
        >[][] /** Very simple customization for better debugging, note the cloning in implementation */;
    ...
    export const spreadMatcher = <T extends Entity, P extends Entity>(
        ...
        const roundMatches = matches.map((matches) =>
            matches.map((match) => ({
                entities: match.entities.map((entity) => ({
                    ...entity
                })),
                volume: BigNumber.from(match.volume)
            }))
        ) as Match<T, P>[][];
